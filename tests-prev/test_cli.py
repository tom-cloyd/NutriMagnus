"""
Tests for the numa CLI (numa.py).

Strategy:
  - The interactive menu uses _prompt(), which falls back to rich Prompt.ask
    when stdin is not a tty (the CliRunner case). Input sequences are passed
    as newline-separated strings via runner.invoke(..., input=...).
...
Input sequence notation used in comments:
    Main menu choices: 1=Foods, 2=Recipes, 3=Meals, 4=Summary, s=Settings, q=Quit
    Submenu:           b=Back, q=Quit
"""
...
from numa import app, _parse_portion_input, _volume_hint
...
class TestStartup:
    def test_help_flag(self, runner: CliRunner):
        ...
        assert "numa" in result.output.lower()

    def test_api_key_flag_saves_and_exits(self, runner: CliRunner, tmp_path, monkeypatch):
        config_file = tmp_path / "config.json"
        monkeypatch.setattr(_usda, "_CONFIG_FILE", config_file)
        result = runner.invoke(app, ["--api-key", "MYKEY123"])
        assert result.exit_code == 0
        assert "saved" in result.output.lower()
        assert config_file.read_text().__contains__("MYKEY123")

    def test_quit_from_main_menu(self, runner: CliRunner):
        result = runner.invoke(app, [], input="q\n")
        assert result.exit_code == 0

    def test_invalid_main_menu_choice_reprompts(self, runner: CliRunner):
        # Enter an invalid choice, then quit
        result = runner.invoke(app, [], input="z\nq\n")
        assert result.exit_code == 0
...
class TestFoodsMenu:
    def test_enter_and_back(self, runner: CliRunner):
        # Main: 1 (Foods) → Foods: b (Back) → Main: q (Quit)
        result = runner.invoke(app, [], input="1\nb\nq\n")
        assert result.exit_code == 0
        assert "Foods" in result.output

    def test_search_displays_results(self, runner: CliRunner, monkeypatch):
    ...
    def test_search_shows_nutrient_table(self, runner: CliRunner, monkeypatch):
        _mock_api(monkeypatch)
        result = runner.invoke(app, [], input="1\n1\nchicken\n1\nb\nq\n")
        assert result.exit_code == 0
    ...
    def test_analyze_portion_scales_nutrients(self, runner: CliRunner, monkeypatch):
        _mock_api(monkeypatch)
    ...
    def test_empty_search_query_returns_to_menu(self, runner: CliRunner, monkeypatch):
        _mock_api(monkeypatch)
        # Empty query → no search → back → quit
        result = runner.invoke(app, [], input="1\n1\n\nb\nq\n")
        assert result.exit_code == 0

    def test_list_cached_foods_empty(self, runner: CliRunner):
        result = runner.invoke(app, [], input="1\n4\nb\nq\n")
        assert result.exit_code == 0
        assert "cached" in result.output.lower() or "No foods" in result.output

    def test_list_cached_foods_shows_entry(self, runner: CliRunner, cached_food):
        result = runner.invoke(app, [], input="1\n4\nb\nq\n")
        assert result.exit_code == 0
        assert "Chicken" in result.output

    def test_search_offers_portion_analysis_accept(self, runner: CliRunner, monkeypatch):
    ...
    def test_search_offers_portion_analysis_decline(self, runner: CliRunner, monkeypatch):
    ...
    def test_invalid_portion_input_retries(self, runner: CliRunner, monkeypatch):
...
class TestRecipesMenu:
    ...
    def test_create_recipe(self, runner: CliRunner, monkeypatch, cached_food):
        _mock_api(monkeypatch)
    ...
    def test_create_recipe_saves_ingredients(self, runner: CliRunner, monkeypatch, cached_food):
        _mock_api(monkeypatch)
        inp = "2\n1\nMy Recipe\n\n1\nd\nchicken\n1\n100\nn\nb\nq\n"
        runner.invoke(app, [], input=inp)
    ...
    def test_list_recipe_after_create(self, runner: CliRunner, monkeypatch, cached_food):
        _mock_api(monkeypatch)
        # Create one recipe
    ...
    def test_delete_recipe(self, runner: CliRunner, monkeypatch, cached_food):
        _mock_api(monkeypatch)
        # Create
        runner.invoke(app, [], input="2\n1\nDeleteMe\n\n1\nd\nchicken\n1\n100\nn\nb\nq\n")
    ...
    def test_view_recipe_shows_nutrients(self, runner: CliRunner, monkeypatch, cached_food):
        _mock_api(monkeypatch)
        runner.invoke(app, [], input="2\n1\nChicken Dish\n\n2\nd\nchicken\n1\n200\nn\nb\nq\n")
        ...
        # n=skip protein analysis, n=skip save report, b=back, q=quit
        result = runner.invoke(app, [], input=f"2\n3\n{rid}\nn\nn\nb\nq\n")
        assert result.exit_code == 0
        assert "Protein" in result.output
...
class TestMealsMenu:
    ...
    def test_log_meal_with_food(self, runner: CliRunner, monkeypatch, cached_food):
        _mock_api(monkeypatch)
    ...
    def test_log_meal_saves_food_item(self, runner: CliRunner, monkeypatch, cached_food):
        _mock_api(monkeypatch)
        inp = "3\n1\n2025-03-15\nDinner\n1\nchicken\n1\n200\nd\nb\nq\n"
        runner.invoke(app, [], input=inp)
    ...
    def test_view_meals_empty_date(self, runner: CliRunner):
        result = runner.invoke(app, [], input="3\n2\n2025-01-01\nb\nq\n")
        assert result.exit_code == 0
        assert "No meals" in result.output

    def test_view_meals_shows_entry(self, runner: CliRunner, monkeypatch, cached_food):
        _mock_api(monkeypatch)
        runner.invoke(app, [], input="3\n1\n2025-03-15\nBreakfast\n1\nchicken\n1\n100\nd\nb\nq\n")
        # Extra b: one for the view action loop, one for the meals submenu
        result = runner.invoke(app, [], input="3\n2\n2025-03-15\nb\nb\nq\n")
        assert "Breakfast" in result.output

    def test_view_meals_shows_food_items(self, runner: CliRunner, monkeypatch, cached_food):
        _mock_api(monkeypatch)
        runner.invoke(app, [], input="3\n1\n2025-03-15\nLunch\n1\nchicken\n1\n150\nd\nb\nq\n")
        result = runner.invoke(app, [], input="3\n2\n2025-03-15\nb\nb\nq\n")
        assert result.exit_code == 0
    ...
    def test_view_meals_delete_item(self, runner: CliRunner, monkeypatch, cached_food):
        _mock_api(monkeypatch)
        runner.invoke(app, [], input="3\n1\n2025-03-15\nLunch\n1\nchicken\n1\n200\nd\nb\nq\n")
        ...
        result = runner.invoke(app, [], input=f"3\n2\n2025-03-15\nd\n{mid}\n{iid}\nb\nb\nq\n")
        assert result.exit_code == 0
    ...
    def test_analyze_meal_shows_nutrients(self, runner: CliRunner, monkeypatch, cached_food):
        _mock_api(monkeypatch)
        runner.invoke(app, [], input="3\n1\n2025-03-15\nLunch\n1\nchicken\n1\n100\nd\nb\nq\n")
    ...
    def test_delete_meal(self, runner: CliRunner, monkeypatch, cached_food):
        _mock_api(monkeypatch)
        runner.invoke(app, [], input="3\n1\n2025-03-15\nLunch\n1\nchicken\n1\n100\nd\nb\nq\n")
...
class TestSummaryMenu:
    ...
    def test_today_summary_no_meals(self, runner: CliRunner):
        result = runner.invoke(app, [], input="4\n1\nb\nq\n")
        assert result.exit_code == 0
        assert "No meals" in result.output

    def test_summary_for_date_shows_totals(self, runner: CliRunner, monkeypatch, cached_food):
        _mock_api(monkeypatch)
    ...
    def test_recent_days_shows_dates(self, runner: CliRunner, monkeypatch, cached_food):
        _mock_api(monkeypatch)
        runner.invoke(app, [], input="3\n1\n2025-03-15\nLunch\n1\nchicken\n1\n100\nd\nb\nq\n")
        result = runner.invoke(app, [], input="4\n3\nb\nq\n")
        assert result.exit_code == 0
...
class TestSettingsMenu:
    ...
    def test_set_api_key(self, runner: CliRunner, tmp_path, monkeypatch):
        config_file = tmp_path / "config.json"
        monkeypatch.setattr(_usda, "_CONFIG_FILE", config_file)
    ...
    def test_change_theme(self, runner: CliRunner, tmp_path, monkeypatch):
        theme_file = tmp_path / "theme"
        monkeypatch.setattr(__import__("numa"), "_THEME_FILE", theme_file)
...
class TestUserProfileSettings:
    def test_profile_option_visible_in_settings(self, runner: CliRunner):
        """Settings menu must show a User profile option."""
        result = runner.invoke(app, [], input="s\nb\nq\n")
        assert result.exit_code == 0
        assert "profile" in result.output.lower()

    def test_set_profile_saves_file(self, runner: CliRunner, tmp_path, monkeypatch):
    ...
    def test_set_profile_shows_calorie_target(self, runner: CliRunner, tmp_path, monkeypatch):
    ...
    def test_invalid_age_reprompts(self, runner: CliRunner, tmp_path, monkeypatch):
    ...
    def test_profile_status_shows_not_set(self, runner: CliRunner, tmp_path, monkeypatch):
    ...
    def test_profile_status_shows_details_when_set(self, runner: CliRunner, tmp_path, monkeypatch):
        """After setting a profile, Settings menu shows age/sex/weight in status line."""
        pf = tmp_path / "profile.json"
        monkeypatch.setattr(_profile, "_PROFILE_FILE", pf)
        # Set profile first
        runner.invoke(app, [], input="s\n6\n40\nf\n62\n165\n2\nb\nq\n")
...
class TestDailySummaryRDA:
    def test_no_profile_shows_tip(self, runner: CliRunner, monkeypatch, cached_food, tmp_path):
    ...
    def test_with_profile_offers_rda_comparison(
        self, runner: CliRunner, monkeypatch, cached_food, tmp_path
    ):
