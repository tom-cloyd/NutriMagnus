...
Last quarterly glossary audit: 2026-09-16 (first run — see Maintenance section).

Last annual static-dataset check: 2026-09-20 (first run — see Maintenance section). Found and applied a real upgrade: Ciqual 2025 (3,484 foods / 74 constituents, +298 foods over the bundled Ciqual 2020's 3,186) supersedes the bundled edition — `ciqual_data.json` regenerated from it, `scripts/build_ciqual_data.py` updated for the new file's reformatted (multi-line) header cells and renamed data sheet ("compo" → "food composition"). AFCD (Release 3) and CoFID (2021) both confirmed still current — no newer edition published for either.
...
| POST | `/recipe/{recipe_id}/instructions` | Save the recipe's instructions (the recipe page's own inline editor; the Edit Recipe page saves instructions, introduction and notes together through `/recipe/{id}/edit`) |
...
**Leave guard ("Data-entered safety").** A separate inline script in `base.html`, placed above the per-page scripts block so page scripts can build on it, watches every form marked `data-leave-guard="<what it edits>"`. Typing in one marks it dirty; clicking any in-app link while it is dirty is intercepted and the user is asked whether to save first (OK = `fetch()` POST the form, then follow the link; Cancel = a second confirm offering to leave without saving, or stay and keep editing). A `beforeunload` listener covers tab close/refresh, which cannot be intercepted this way. Two optional attributes: `data-leave-guard-alert="<element id>"` names a "Saved" banner to remove the moment the form is touched (it describes the page-load state, not what is now typed in), and `data-leave-guard-ignore` on any element *inside* the form excludes fields that actually submit elsewhere via the HTML `form=""` attribute (`recipe_edit.html`'s Introduction block). The script exposes `window.numaLeaveGuard.isDirty(form)` / `.markClean(form)` for page scripts that need the same state — `recipe_edit.html` uses it to save pending Recipe details before an ingredient-add or Introduction submit. Every page carrying a guarded form shows the matching user-facing note via the `_failsafe_note.html` macro; note and attribute always ship together, and `tests/test_web.py` asserts that for each such page. Currently: Edit Recipe (Recipe details), a meal page (Rename / change date), Edit Custom Profile (food details), and Annotate a Food (GI / DIAAS estimates).
...
**Manual updates (separate from program updates).** `numa_app/services/manual_update.py` checks a rolling prerelease, `manual-latest`, on GitHub for a newer signed `user-manual.html`; `scripts/publish_manual.py` builds, signs (Ed25519, key in `~/.config/numa-signing/`, public key baked into the module), and uploads it. Home shows its own banner (own dismissal pref, `manual_notice_dismissed_stamp`); `POST /manual-update-now` verifies and installs into `<data dir>/manual/`. `/manual` serves the downloaded copy only if it re-verifies and its stamp is newer than the baked-in manual's; the `rebuild_manual_if_stale()` step applies only to the baked-in copy. The manifest's `requires_program` drives the "describes newer features" warning. Never remove or rename a manual anchor (older programs link into newer manuals).
...
Recipe edit page: add/remove/reorder ingredients (food or sub-recipe), inline ingredient edit, instructions/introduction/notes text, servings, and total weight/volume. The whole Recipe details section is one form with one "Save recipe details" button, which sits on the `<summary>` disclosure row and targets the form by id via the HTML `form=""` attribute (base.html's unsaved-changes tracker falls back to that lookup so such a form keeps its badge; a click handler stops the button from also collapsing the `<details>`). Introduction used to be a second form with its own save button and route — a second Save button on one page was a recurring source of "which one do I press?". "Notes and documentation" (`recipes.notes`) saves with the main Recipe details form and renders only when non-empty. The "Add Ingredient" search form uses the same `data-persist-search` mechanism as `meal.html`.

**Placement rule for "Notes and documentation"** (and any later field of the same kind): on an *edit* page it sits directly beneath the Instructions field, because that is where you are thinking about the recipe's own text; on every *output* page it goes last, after the analysis, because it is reference material about the recipe rather than part of the analysis. Currently: Edit Recipe (below Instructions), recipe detail page (last section), printable analysis (last block, `print_sections.PRINT_SECTION_LABELS` order), and the translation preview (last block).
...
`tests/test_estimate_aa_properties.py`, `tests/test_diaas_properties.py`, and `tests/test_complements_properties.py` use the [Hypothesis](https://hypothesis.readthedocs.io/) library instead of hand-picked example inputs: each generates many random-but-plausible inputs per run and asserts an invariant holds for all of them (AA/protein ratio preservation under scaling; DIAAS/digestible-protein totals staying in-range; a complement suggestion never making its targeted amino acid worse, never claiming to close a gap it doesn't, and never being ranked below a strictly worse suggestion). Complements the file-by-file table above rather than replacing it — these three files are also listed there.

### Browser-level end-to-end tests

`tests/e2e/` uses [Playwright](https://playwright.dev/) to drive a real, isolated instance of the app in an actual browser — currently covering Food Search, Analyze a Food Portion, and a meal's Add Food panel's behind-the-scenes async refresh (confirming the JS that re-fetches/re-sorts once external sources respond actually runs, not just that the page ships the right code). Runs weekly (Thursday 06:00 UTC, just ahead of the sweep) and on demand, separately from the rest of the suite (which runs on every change) since a real browser adds real wall-clock time.

### Mutation testing

Covered in full in the Maintenance section below ("Quarterly mutation-testing rotation, plus a weekly churn check") — this note is just a pointer so it's discoverable from here too. Short version: `mutmut` deliberately plants small errors in the source (a `+` swapped for a `-`, a boundary check off by one) and reruns the targeted tests to see whether anything notices; a mutant nothing catches is a real, measured coverage gap that a normal "tests pass" run can't reveal on its own.
...
- **Quarterly glossary audit** — due if "Last quarterly glossary audit" is 3+ months ago (or has never been run). See that section below.
...
### Quarterly glossary audit (every 3 months)

The weekly sweep's item 9 (Glossary coverage) only scans what the last two weeks of changelog entries introduced — real but slow drift (a term that crept into the manual gradually, never as one flagged changelog entry) can slip past that indefinitely, the same gap that motivates the monthly full manual audit versus its own two-week-scoped weekly item 5. This check is the glossary's equivalent: a full scan of the whole document, not just recent changes.

**Cadence: every 3 months**, not weekly or monthly — new terminology doesn't accumulate fast enough in a single-author manual to need checking more often, and `scripts/audit_glossary.py` (below) makes each run cheap regardless of how long it's been.
...
**First run (2026-09-16):** found and closed six real gaps — ALA, DHA, EPA (the omega-3 fatty acid trio, tracked nutrients with no prior definition), IU (International Units, used in the custom-food-profile vitamin-entry instructions), UL (Tolerable Upper Intake Level, the RDA/AI/DRI family's missing member), and IAA (the FAO's own term for EAA, used throughout Appendix G's worked example without local re-explanation). CSV was added the same day, found by the user directly rather than this script (script didn't exist yet). Twenty other candidates reviewed and correctly left out: ID/API/JSON/PDF/PNG/SVG/US/README (too generic or well-known to need definition) and the six-database source-name cluster (CIQUAL/AFCD/CNF/SR/UK/NIH/ANSES/FSANZ/DHSC/FNDDS, all already explained in place in the Food data section). UPC/EAN (barcode standards) were the one genuinely borderline case — flagged to the user, who chose to leave them undefined, same reasoning as PDF/PNG/SVG.

### Annual static-dataset edition check

AFCD, CoFID, and CIQUAL (`afcd_data.json`, `cofid_data.json`, `ciqual_data.json`) are bundled snapshots of official periodic releases — AFCD "Release 3," CoFID 2021, CIQUAL 2025 — built once by `scripts/build_afcd_data.py`/`build_cofid_data.py`/`build_ciqual_data.py` from a downloaded spreadsheet, not live APIs. They can't silently break the way the quarterly check's three live APIs can, but they can quietly go stale: the publishing body (FSANZ, McCance and Widdowson/PHE, ANSES) eventually releases a newer edition, and NuMa keeps serving the old one indefinitely with nothing to flag it.
