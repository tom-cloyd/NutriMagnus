...
`tests/e2e/` uses [Playwright](https://playwright.dev/) to drive a real, isolated instance of the app in an actual browser — currently covering Food Search, Analyze a Food Portion, and a meal's Add Food panel's behind-the-scenes async refresh (confirming the JS that re-fetches/re-sorts once external sources respond actually runs, not just that the page ships the right code). Runs weekly and on demand, separately from the rest of the suite (which runs on every change) since a real browser adds real wall-clock time.
